...
from data.snli_ve_dataset import snli_ve_dataset
...
def create_dataset(dataset, config, min_scale=0.5):
    ...
    if dataset=='pretrain':
        ...
        val_dataset = vqa_dataset(transform_test, config['ann_root'], config['vqa_root'], config['vg_root'], split='valid')
        test_dataset = vqa_dataset(transform_test, config['ann_root'], config['vqa_root'], config['vg_root'], split='test')
        return train_dataset, val_dataset, test_dataset

    elif dataset=='snli_ve':
        train_dataset = snli_ve_dataset(transform_train, config['image_root'], config['ann_root'], 'train')
        val_dataset = snli_ve_dataset(transform_test, config['image_root'], config['ann_root'], 'val')
        test_dataset = snli_ve_dataset(transform_test, config['image_root'], config['ann_root'], 'test')
        return train_dataset, val_dataset, test_dataset
